<?php
// admin/edit_surat.php — Edit surat keluar yang sudah pernah dibuat.
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['aksi'] ?? '') === 'simpan_edit_surat') {
    ...
    if (!$isPreviewOnly) {
        ...
        try {
            ...
            $fileHasilBaruLokal = arp_dengan_template_sementara($kode['drive_file_id'], function ($pathTemplateLokal) use ($dataForm, $items, $nomorBaru, $blocksData, $kode, $tujuanManual, $ringkasanDisertakan, $revisiKeDipakai, $rowsAkumulasi, $rowsRincianOrang) {
...
foreach ($fields_dinamis as $f) {
    ...
    $nilai_dinamis[$f['field']] = $_POST['dinamis'][$f['field']]
        ?? ($isiDataAsli[$f['field']] ?? ($isTanggalField ? date('Y-m-d') : ($isWaktuField ? date('H:i') : '')));
}
...
$nilaiRincianOrang = $_POST['rincian_orang'] ?? ($isiDataAsli['__rincian_orang'] ?? []);
...
?>
            ...
            <?php if ($kodeTerpilih && !$file_template_hilang && $kodeTerpilih['format'] === 'word_pdf'): ?>
                ...
                <form method="POST" action="edit_surat.php?id=<?= (int) $surat_id ?>" id="form-edit-surat">
                    ...
                    // Prioritaskan invoice yang baru dikirim lewat POST (mis. saat "Update Preview")
// dibanding data lama tersimpan, supaya pilihan tidak balik ke versi lama.
                    $invoiceSumberIdTersimpan = (int) ($_POST['invoice_sumber_id'] ?? ($isiDataAsli['invoice_sumber_id'] ?? 0));
                    $ikutiNomorInvoiceChecked = $isPostEditSurat
                        ? isset($_POST['ikuti_nomor_invoice'])
                        : (bool) ($isiDataAsli['ikuti_nomor_invoice'] ?? false);
                    ?>
                    ...
                    <?php if ($butuhInvoiceSumber): ?>
                        <div class="arp-static-info-box text-xs mb-3" style="position:relative;">
                            ...
                            <div style="width:100%;">
                                ...
                                <input type="text" class="form-control-custom" id="cari-invoice-sumber" autocomplete="off"
                                    placeholder="Ketik nomor surat atau nama tujuan invoice..." required
                                    <?= $invoiceSumberIdTersimpan > 0 ? 'readonly' : '' ?>>
                                ...
                                <small class="text-secondary text-xs d-block mt-2">
                                    ...
                                    grand total, total bayar, terbilang, dst) akan otomatis di-refresh dari invoice
                                    yang dipilih di sini saat perubahan disimpan.
                                    <?= $invoiceSumberIdTersimpan > 0 ? ' Invoice sumber sudah terkunci dan tidak dapat diubah.' : '' ?>
                                </small>
                        ...
                        <script>
                            (function () {
                                ...
                                function tampilkanInfoInvoice(data) {
                    ...
                    <?php if (empty($fields_dinamis) && empty($fields_tabel) && empty($fields_blok) && empty($fields_akumulasi) && empty($fields_rincian_orang)): ?>
                        <div class="alert alert-danger-custom text-xs">
                            <i class="bi bi-exclamation-triangle-fill"></i>
                        ...
                        <template id="template-baris-item">
                            <tr class="baris-item" data-baris-index="__IDX__">
                                ...
                                <?php foreach ($fields_tabel as $kolom): ?>
                                    ...
                                    ?>
                                    <td>
                                        <input type="<?= $tipeInputKolom ?>" name="items[__IDX__][<?= e($kolom['field']) ?>]"
                                            data-kolom="<?= e($kolom['field']) ?>" <?= $isHarga ? 'data-tipe="harga"' : '' ?>
                                            placeholder="<?= $isTanggalKolom || $isWaktuKolom ? '' : e($placeholderKolom) ?>"
                    ...
                    <?php if ($adaRincianOrangTemplate): ?>
                        <div class="mt-3">
                                        ...
                                        <?php foreach ($nilaiRincianOrang as $i => $b): ?>
                                            <tr class="baris-rincian-orang" data-baris-index="<?= (int) $i ?>">
                                                ...
                                                <td><input type="text" name="rincian_orang[<?= $i ?>][nama_orang]"
                                                        class="form-control-custom" value="<?= e($b['nama_orang'] ?? '') ?>"></td>
                                                ...
                                                <td><input type="text" name="rincian_orang[<?= $i ?>][kategori]"
                                                        class="form-control-custom" value="<?= e($b['kategori'] ?? '') ?>"></td>
                            ...
                            <button type="button" id="tombol-tambah-rincian-orang" class="btn btn-outline-primary btn-sm mt-2">
                                <i class="bi bi-plus-lg"></i> Tambah Baris
                            </button>
